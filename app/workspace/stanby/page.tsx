...
const API_KEY = process.env.NEXT_PUBLIC_OPENTOK_API_KEY;
const SESSION_ID = process.env.NEXT_PUBLIC_OPENTOK_SESSION_ID;
...
const Page = () => {
  ...
  const handleBlurChange = (value: any) => {
    setBlurAmount(value);
  };
  ...
  return (
    <div className="px-24 mt-10">
      <Link href="/">
        <p className="border-b-blue-500 text-3xl font-bold top-0 outline-2 cursor-pointer">
          Home
        </p>
      </Link>
      <div className="flex justify-center">
        <div
          id="myCamera"
          className="bg-gray-600 w-[1244px] h-[732px] rounded-2xl"
        ></div>
      </div>
      <div className="flex justify-center mt-5">
        <div className="flex justify-end items-center w-[1244px]">
          <Link
            href="/workspace/stanby/video"
            className="bg-gray-500 w-[190px] h-[190px] rounded-full flex items-center justify-center text-white mr-4"
          >
            JOIN
          </Link>
          <Slider
